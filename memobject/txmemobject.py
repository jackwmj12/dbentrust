...
class MemConnectionManager:
    ...
    @classmethod
    def setConnection(cls,connection : BaseRedisProtocol):
        '''
        设置redis连接池
    ...
    @classmethod
    def getConnection(cls) -> BaseRedisProtocol:
        '''
        获取redis连接池
        '''
...
class MemAdmin(MemObject):
    ...
    def scan_leafs(self, leaf, start=0, count=1000):
        ...
        '''
        leafs = []
        keys = yield self.scan_leafnames(leaf, start, count)
        for key in keys:
...
class MemSortedSet(MemCache):
    ...
    def addMemByEntrust(self,mem,value):
        ...
        if isinstance(mem,MemCache):
            ...
            raise Exception("mem type error")
        return MemConnectionManager.getConnection().zadd(self.key,member=mem,score=value)
    ...
    def getMemsByIndex(self,start, end, desc=False, withscores=True,
        ...
        return MemConnectionManager.getConnection()._zrange(
            self.key,start, end,withscores=withscores,reverse=desc)
    ...
    def getMemsByMemRange(self,min, max, start=None, num=None):
        ...
        :param
        '''
        return MemConnectionManager.getConnection()\
            .zrangebylex(self.key, min, max, start=None, num=None)
    ...
    def getMemsByScoreSortedByScore(self,max, min,start=0, num=100, withscores=False,score_cast_func=float):
        ...
        return MemConnectionManager.getConnection()\
            .zrevrangebyscore(
            self.key, max, min,withscores,offset=start, count=num
        )
